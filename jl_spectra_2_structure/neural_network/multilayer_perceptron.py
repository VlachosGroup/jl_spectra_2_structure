...
class BaseMultilayerPerceptron(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def _forward_pass(self, activations):
        ...
        for i in range(self.n_layers_ - 1):
            activations[i][abs(activations[i][...])<2**-500] = 0
            self.coefs_[i][abs(self.coefs_[i][...])<2**-500] = 0
    ...
    def _backprop(self, X, y, activations, deltas, coef_grads,
        ...
        if loss_func_name in ['wasserstein_loss','mixed']:
            ...
            Tcum = np.cumsum(y,axis=-1)
            PminusT = Pcum-Tcum
            deltas[last] = 2*P*(np.cumsum(PminusT[:,::-1],axis=-1)[:,::-1]
        ...
        try:
            for it in range(self.max_iter):
                ...
                if self._no_improvement_count > 2:
                    # not better than last 2 iterations by tol.
                    # stop or decrease learning rate
                    if early_stopping:
                        msg = ("Validation score did not improve more than "
                               "tol=%f for 2 consecutive epochs." % self.tol)
                    else:
                        msg = ("Training loss did not improve more than tol=%f"
                               " for 2 consecutive epochs." % self.tol)
    ...
    def _update_no_improvement_count(self, early_stopping, X_val, y_val):
        if early_stopping:
            ...
            """
            if self.loss == 'kl_div_loss':
                y_pred = self.predict(X_val)
                self.validation_scores_.append(-1*LOSS_FUNCTIONS[self.loss](y_val, y_pred))
            else:
            """
...
class MLPRegressor(BaseMultilayerPerceptron, RegressorMixin):
    ...
    ----------
    Hinton, Geoffrey E.
        "Coselfectionist learning procedures." Artificial intelligence 40.1
        (1989): 185-234.
