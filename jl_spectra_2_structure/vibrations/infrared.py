    ...
    *get_frequencies()*, *get_spectrum()* and *write_spectra()*
    ...
    def read(self, method='standard', direction='central'):
        ...
        if self.imagetype == 'atoms':
            ...
            self.dipole_zero = (np.sum(self.atoms[0].dipole**2)**0.5) / units.Debye
        ...
        dpdQ = dpdQ.T
        intensities = np.array([np.sum(dpdQ[j]**2) for j in range(ndof)])
        # Conversion factor:
    ...
    def get_spectrum(self, start=800, end=4000, npts=None, width=4,
                     ...
                     intensity_unit='(D/A)2/amu', normalize=False,freqindices=None):
        ...
        if freqindices is None:
            freqindices=range(0,len(frequencies))
        return self.fold(frequencies[freqindices], intensities[freqindices],
